...
void main() {
  ...
  test('header explains the \$-rename scheme (do not edit this file)', () {
    expect(code, contains('do not edit it directly'));
    expect(code, contains('Identifier scheme (GraphQL name -> Dart name), and why'));
    // Every rename rule mentioned in naming.dart gets a line here.
    expect(code, contains('_eq'));
    expect(code, contains(r'type$'));
    expect(code, contains(r'Object$'));
    expect(code, contains(r'users$'));
    expect(code, contains('Enum constants are lowerCamelCased'));
    expect(code, contains(r'Accessor.$typename'));
    // constant_identifier_names is no longer needed: enum constants are
    // always lowerCamelCase, never SCREAMING_CASE. (non_constant_identifier_names
    // is unrelated and still emitted.)
    expect(
      code,
      contains(
