...
| `--scalar` | no, repeatable | Custom scalar mapping: `Name=DartType[:converterExpr]`. See [Scalar mapping](#scalar-mapping) below. |
...
- **Built-in**: `DartType` is `DateTime` and there is no `converterExpr` —
  reads with `DateTime.parse`, writes with `.toIso8601String()`.
- **Generic**: a `converterExpr` names a class with static
  `T parse(String)` and `String serialize(T)` methods, e.g.
  `--scalar Money=Decimal:MoneyConverter` emits
  `scalarAs<Decimal, String>('amount', MoneyConverter.parse)` for the getter
  and `MoneyConverter.serialize(v)` for the setter/arguments. A converter is
  required unless `DartType` is `DateTime` — the generator has no way to
  guess how to (de)serialize an arbitrary type otherwise.
...
<Aside type="tip">
The generated file itself starts with a header comment listing every rule
below in one place (GraphQL name → Dart name, and why), right above the
`// GENERATED CODE - DO NOT MODIFY BY HAND` line, so a reader never has to
go looking for why `type$` or `$typename` is spelled that way.
</Aside>
