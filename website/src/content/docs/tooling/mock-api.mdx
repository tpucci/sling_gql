...
`me` returns a static viewer object (id `viewer-1`, name `Mira Vance`, agency
...
`favoriteCount` and `favorites` are computed at request time, not cached, so
...
`filter: { favorite: true }` returns only launches where `l.favorite ===
true` (server-side comparison). The example app's Favourites segment uses
this. Because the argument set differs from the plain `launches(first: N)`
query, the sling_gql cache stores them as separate entries — switching back
to All is served from cache (zero requests).
...
`toggleFavorite` is what the example's detail-screen heart calls (see
[Mutations](/sling_gql/guides/mutations/)); favourites live in the server's memory, so they
...
[Roadmap](/sling_gql/internals/roadmap/)):
